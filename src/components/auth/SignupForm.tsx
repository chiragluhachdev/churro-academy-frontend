...
/** Cheap, honest strength read: length plus variety of character classes. */
...
export function SignupForm() {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    // Frontend-only for now — no account is created.
    setSubmitting(true);
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-5">
        ...
        <div>
          ...
          {password.length > 0 && !errors.password && (
            <div className="mt-2.5 flex items-center gap-3">
              ...
              <span className="text-muted w-14 text-right text-[0.75rem]">
                {strength.label}
              </span>
            </div>
          )}
        ...
        <div>
          ...
          {errors.terms && (
            <p role="alert" className="mt-2 text-[0.78rem] text-red-600">
              {errors.terms}
            </p>
          )}
        </div>
      ...
      <p className="text-muted mt-5 text-[0.75rem] leading-relaxed">
        Demo build — no account is actually created yet.
      </p>
    </div>
  );
}
